defmodule Turtles.Turtle do
  ...
  def start_link(id) do
    GenServer.start_link(@name, %{id: id, next_instruction: "sleep(3)", result: ""})
  end
  ...
  def set_next_instruction(pid, next_instruction) do
    GenServer.call(pid, {:set_next_instruction, next_instruction}, @timeout)
  end

  def post_result(pid) do
    GenServer.call(pid, :post_result, @timeout)
  end

  def handle_call(:fetch_next_instruction, _from, state) do
    {:reply, state.next_instruction, %{state | next_instruction: "sleep(3)"}}
  end

  def handle_call({:set_next_instruction, next_instruction}, _from, state) do
    {:reply, :ok, %{state | next_instruction: next_instruction}}
  end

  def handle_call({:post_result, _result}, _from, %{result: ""}), do: :ok
  def handle_call({:post_result, result}, _from, %{result: result}), do: :ok

  def handle_call({:post_result, _given_result}, _from, %{result: _expected_result}) do
